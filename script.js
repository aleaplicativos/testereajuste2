function adicionarLinha() {
  ...
  for(let i=0;i<6;i++) {
    ...
    if (i === 2) cell.classList.add("sem-arred");
    if (i === 2) cell.style.display = "none";
    cell.textContent="-";
  }
  ...
  btn.onclick=()=>{novaLinha.remove();atualizarTotais();};
...
function atualizarLinha(linha,indice,dataInicio,dataFim){
  ...
  linha.cells[7].textContent=vUnitUpd.toFixed(10).replace('.', ',');
...
function atualizarTotais(){
  ...
  Array.from(rows).forEach(r=>{
    tA+=desformatarMoeda(r.cells[5].textContent);
    tU+=desformatarMoeda(r.cells[8].textContent);
    tD+=desformatarMoeda(r.cells[9].textContent);
  });
...
function copiarTabela(){
  const t=document.getElementById("tabelaItens"),
...
function toggleColunaSemArredondamento() {
  ...
  for (let i = 1; i < rows.length; i++) {
    const cell = rows[i].getElementsByClassName("sem-arred")[0];
